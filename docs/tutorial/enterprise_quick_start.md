...
|  节点4      | 127.0.0.1:30304| 127.0.0.1:8549/:20204 | 机构C | 群组1、2 |
|  节点5      | 127.0.0.1:30305| 127.0.0.1:8550/:20205 | 机构C | 群组1、2 |
...
# 由于B、C机构不需要生成创世区块，因此只需交换peers至机构B、C
generator-A$ cp -r ./agencyA_send/peers.txt ~/generator-B/peersA.txt
```
...
```bash
# 生成机构A所属节点
generator-A$ ./generator --build_install_package ./agencyB_send/peers.txt ./nodeA
# 启动节点
...
```bash
# 查看机构A节点连接文件peersA
generator-B$ cat ./peersA.txt
# 生成机构C所属节点
generator-B$ ./generator --build_install_package ./peersA.txt ./nodeB
# 启动节点
...
# 交换证书与peers至机构A
generator-C$ cp -r ./agencyC_send ~/generator-A/
```
...
```bash
# 将机构C证书放置于meta文件夹
generator-A$ cp ./agencyC_send/* ./meta/
# 生成群组2创世区块
generator-A$ ./generator --create_group_genesis ./data
# 将群组2创世区块发送给机构C
generator-A$ cp ./data/group.2.genesis ~/generator-C/meta/
# 添加群组2配置文件至已有节点
generator-A$ ./generator --add_group ./data/group.2.genesis ./nodeA
# 添加机构C节点连接文件peers至已有节点
generator-A$ ./generator -add_peers ././agencyC_send/peers.txt ./nodeA
# 将节点连接文件peers发送给机构C
...
```bash
# 查看机构A节点连接文件peersA
generator-C$ cat ./peersA.txt
# 生成机构C所属节点
generator-C$ ./generator --build_install_package ./peersA.txt ./nodeC
# 启动节点
