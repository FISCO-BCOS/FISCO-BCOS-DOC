...
.. admonition:: v2.0.0-rc1升级到v2.0.0-rc2
    ...
    - **全面升级** ：参考 `安装 <../tutorial/installation.html>`_ 搭建新链，重新向新节点提交所有历史交易，升级后节点包含v2.0.0-rc2新特性
...
* [并行计算模型](../app_dev/transaction_parallel.md)：可并行合约开发框架、交易并行执行引擎（PTE）
* [分布式存储](../blockchain_dev/distributed_storage.md)：amdb-proxy、SQLStorage

**版本优化**

* 优化了[区块打包交易数](../blockchain_dev/configuration.html#id15)的逻辑，根据执行时间动态的调整区块打包交易数
* 优化了区块同步的流程，让区块同步更快
...
* 加入[兼容性配置](../blockchain_dev/configuration.html#id7>)
* 交易编码中加入chainID和groupID
* 交易中加入二进制缓存
* 创世块中加入timestamp信息
* 增加了一些precompile的demo
* 支持用[Docker搭链](../tools/build_chain.md)
* 删除不必要的日志
* 删除不必要的重复操作
