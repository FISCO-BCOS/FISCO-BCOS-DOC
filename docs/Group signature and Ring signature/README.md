...
|目录           | 说明                                                                                |111 |
+===============+=====================================================================================+=================+
|dist/apps      | 存放web3sdk编译生成的jar包web3sdk.jar                                               | 111|
+---------------+-------------------------------------------------------------------------------------+-----------------+
|               |  - web3sdk: 调用web3sdk.jar执行web3sdk内方法(如部署系统合约、调用合约工具方法等)    | 111|
|dist/bin       |  - compile.sh: 将dist/contracts目录下的合约代码转换成java代码，供开发者使用         | 111|
+---------------+-------------------------------------------------------------------------------------+-----------------+
|dist/conf      | 配置目录, 用于配置节点信息、证书信息、日志目录等                                    | 111|
+---------------+-------------------------------------------------------------------------------------+-----------------+
|dist/contracts | 合约存放目录，compile.sh脚本可将存放于该目录下的合约代码转换成java代码              | 111|
+---------------+-------------------------------------------------------------------------------------+-----------------+
|dist/lib       | 存放web3sdk依赖库的jar包                                                            | 111|
+---------------+-------------------------------------------------------------------------------------+-----------------+
```
