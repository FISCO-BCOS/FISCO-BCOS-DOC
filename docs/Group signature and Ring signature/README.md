...
- **Case 1**： Both members (Consumer user) and subordinate organizations in the parent organization on behalf of the parent organization place group signature on the blockchain. When verifiers verifies the group signature, they can only identify the group, that the signature belongs to, but cannot identify the individual signatory. So group signature can maintain signatory's anonymity and ensure the signature has not been modified.
Group signature can be used for auctions, anonymous deposits and credit checks etc.
- **Case 2**：The business user sends the generated group signature on the consortium blockchain organization (such as webank) through AMOP, and the organization places the collected group signatures (such as bidding, reconciliation) on the blockchain. The verification process maintains signatory's anonymity. But the regulator can trace the signatory's identity through trusted third parties.
...
- **Case 1 (Anonymous voting)**：Members (Consumer user) in the organization signs the vote and place the signature on the blockchain via the trusted agent(such as webank), When verifiers verifies the ring signature, they can only identify the individual agent the signature belongs to, but cannot identify particular signatory(voter).

- **Other cases (Anonymous deposit, credit checks etc.)**：The cases are similar to group signature's anonymous deposit and credit checks cases, but the difference is that NO ONE can trace the signatory's identity.

- **Anonymous transaction**：In UTXO model, the ring signature algorithm can be used to anonymize transactions, and no one can trace the sender and receiver of the transaction.
...
FISCO BCOS supports configuring whether to enable the ethcall for group signature & ring signature or not.(by default ethcall is disabled). 
...
- dos2unix && lsof: Use for file handling in linux, during windows to linux file upload.

Centos and ubuntu OS can use below commands to install these dependencies:
...
(2) After enabling ethcall, **you cannot stop ethcall service，otherwise the interface would be unreachable when performing the verification, and will cause abnormal exit**

If ethcall is stopped by mistake, FISCO BCOS can be reverted back to the version used to enable the ethcall.

<br>

**(3) The FISCO BCOS's version must be consistent**

If a node enables group/ring signature verification service, then all the other nodes must enable it as well. Otherwise the nodes which have not enabled verification service will abnormal exit.
<br>
