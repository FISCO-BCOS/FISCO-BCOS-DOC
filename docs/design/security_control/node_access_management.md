...
2.3列出的五个<font color=#FF0000>配置项及系统模块关系图</font>如上，箭头方向A->B表示B模块依赖A模块的数据，同时B模块晚于A模块初始化。
...
对于<font color=#FF0000>**网络可改配置文件**</font>，节点准入管理涉及**P2P节点连接列表`[p2p]`**、**节点证书`[secure]`**、**CA黑名单`[crl]`**。`[crl]`可缺省。对于<font color=#FF0000>**群组固定配置文件**</font>举例如下，节点准入管理涉及**群组节点初始列表[consensus]**。以上两配置文件示例可参考[《节点配置说明文档》](http://wiki.weoa.com/books/fisco-bcos/page/group-configuration-instructions)，此处不赘述。
