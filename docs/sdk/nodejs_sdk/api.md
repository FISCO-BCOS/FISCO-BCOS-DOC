...
- **Web3jService**：提供访问FISCO BCOS 2.0节点[JSON-RPC](../../api.md)接口支持；提供部署及调用合约的支持。
- **PrecompiledService**：
...
**功能**：访问FISCO BCOS 2.0节点[JSON-RPC](../../api.md)；部署合约；调用合约
...
| sendRawTransaction | 发送交易 | 交易的RLP编码  | 同上 |
| deploy |  部署合约  | 合约路径<br>输出路径| 同上 |
| call | 调用合约  | 合约地址<br>调用接口的ABI<br>参数列表 | 同上 |
