...
- `channel_listen_port`: Channel端口，对应到[Web3SDK](../sdk/web3sdk/java_sdk.html#id2)配置中的`channel_listen_port`；
...
- `node.*`: 节点需连接的所有节点`IP:Port`或`DomainName:Port`。该选项支持域名，但建议需要使用的用户[手动**编译源码**](https://fisco-bcos-documentation.readthedocs.io/zh_CN/latest/docs/blockchain_dev/get_executable.html#id2)。
...
- `[flow_control].outgoing_bandwidth_limit`：节点出带宽限制，单位为`Mbit/s`，当节点出带宽超过该值时，会暂缓区块发送，也会拒绝客户端发送的[AMOP](../app_dev/amop_protocol.md)请求，但不会限制区块共识和交易广播的流量，**该配置项默认关闭，若要打开流量限制功能，请将`outgoing_bandwidth_limit`配置项前面的`;`去掉**。
...
.. important::
    ...
    - 由于genesis配置要求群组内所有节点一致，建议使用 `开发部署工具 build_chain <../tools/build_chain.html>`_ 生成该配置
```
...
下面为rPBFT模式下开启[Prepare包结构优化](../blockchain_dev/configuration.html#pbft-prepare)后，负载均衡相关配置：
