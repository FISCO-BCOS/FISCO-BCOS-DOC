...
.. important::
    ...
    备注：fisco bcos的系统设计和我们平常接触的常规系统设计略有不同，在这里如果不加以区分，可能存在思维定性的误区，导致理解偏差。    
    常规系统通常是授权之后，各个角色用户才拥有对应的操作权限，才可以合规化操作。比如：链管理员授权某个用户具有部署合约权限，这个用户才可以进行部署合约。    
    但fiscobcos 在设计上不同，一开始，如果没有进行合约授权命令操作时候，任何人都可以部署合约，可是一旦链管理员授权某个用户具有部署合约的权限，这个     时候部署合约权限就被锁定，只有授权的用户才可以进行合约部署操作。
```
