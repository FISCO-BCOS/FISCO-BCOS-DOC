...
区块链在运行的过程中，随着上链业务数的增加，会出现链数据量成比例增加和链性能有一定程度衰减的情况。其中性能衰减对业务体验影响较大，并且无法通过增加硬盘容量等硬件方式解决。通过分析，我们可采用<font color="#FF0000">scalable存储模式</font>，通过对写入本地的链上数据按照一定维度进行划分，分别存于不同存储实例，从而控制节点所访问的单个存储实例大小，稳定链处理能力；同时，对本地的存储实例，在实现备份归档后可以有选择地进行删除，控制本地存储的整体数据量。
...
- 设置`[storage].binary_log=true`来启用binlog。如使用build_chain脚本搭链，配置文件会自动打开binlog。

- 参考文末进行amdb-proxy配置，其中设置amdb-proxy访问的数据库为[“数据仓库”](https://data-doc.readthedocs.io/zh_CN/latest/docs/WeBankBlockchain-Data-Stash/index.html)生成的数据库。

### 文件组织

使用scalable存储模式后，数据目录内容如下：

- `nodeX/data/groupX/block/Scalable/blocksDB/`下按起始块高分DB存储区块数据，在配置了“数据仓库”和amdb-proxy后，较旧的子文件夹可以删除。
- `nodeX/data/groupX/block/Scalable/state`存储状态数据，该文件夹不可删除。

## binlog文件

binlog记录了每个区块中的每个交易对区块链状态的修改结果。binlog机制的作用在于：

1. 提供了区块维度的数据操作结果记录；
2. 节点可从binlog文件而非通过原有的同步区块重放交易来恢复数据；
3. 为“数据仓库”的快照构建提供数据来源。

基于1，用户可根据下图描述的binlog协议，通过访问binlog文件（位置`nodeX/data/groupX/block/BinaryLogs/`），获取某个key的历史修改记录。

![](../../images/storage/binlog.png)

binlog的启用通过开关通过群组的ini配置文件中`[storage].binary_log`控制，默认关。如果配置了scalable存储模式，需同步启用binlog，否则启动报错。

## fisco-sync同步数据工具

### fisco-sync简介

使用fisco-sync进行数据同步的流程包括：

1. 新加入网络的节点连接“数据仓库”的数据库，通过fisco-sync工具拉取指定区块的最新数据（区块快照）存储到指定的存储模式；
2. 新节点基于流程1的数据基础上从其他节点同步区块，并执行区块的交易更新新节点自身的状态数据；
3. 新节点的块高追平网络块高，参与共识。
...
2. 修改新节点指定群组的group.X.ini配置文件信息，包括`storage.topic`字段和指定存储模式；
3. 指定验证区块数及群组ID，启动fisco-sync，完成后生成相应的data和log目录；
...
新节点配置文件中的存储模式可选`rocksdb`、`mysql`和`scalable`，前两者拉取的是全量数据（区块数据+状态数据），`scalable`拉取的是经裁剪后的数据（状态数据）。新节点启动`fisco-bcos`时不需修改配置文件。
...
amdb-proxy与节点连接过程，amdb-proxy是client,节点是server，启动过程是amdb-proxy服务主动连接节点，节点只需要配置amdb-proxy关注的topic即可，amdb-proxy需要通过证书准入。
