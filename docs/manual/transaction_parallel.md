...
FISCO BCOS为了满足某些高性能业务场景的需求，提供了并行交易的编程框架。按照并行框架规范编写的合约，能够被FISCO BCOS节点并行地执行。并行合约具有如下特性：

- 高吞吐量：多笔独立交易同时被执行，能够最大限度的利用机器的CPU资源，从而拥有较高的TPS；
- 平行拓展：可以通过提高机器的配置来提升交易执行的性能，以支持不断扩大业务规模。

本文将指导完成如何编写FISCO BCOS并行合约，以及如何部署和执行并行合约。

## 预备知识

### 并行互斥

两笔交易是否能被并行的执行，依赖于这两笔交易是否存在**互斥**。互斥，是指两笔交易各自需要操作合约存储变量的集合存在交集。
...
FISCO BCOS提供了智能合约的并行编程的框架，开发者只需按照框架的规范开发合约，定义好每个合约接口的互斥参数，即可实现能被并行执行的合约。当合约被部署后，FISCO BCOS即可在执行交易前，自动解析互斥对象，让无依赖关系的交易并行执行。
...
在合约中实现 ``` enableParallel() ``` 函数，调用``` registerParallelFunction() ```注册可以并行的合约的接口。同时也需要实现``` disableParallel() ```函数，使合约具备取消并行执行的能力。
