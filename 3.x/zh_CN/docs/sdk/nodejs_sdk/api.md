...
- **Web3jService**：提供访问FISCO BCOS 2.0+节点[JSON-RPC](../../develop/api.md)接口支持；提供部署及调用合约的支持。
- **PrecompiledService**：

    Precompiled合约（预编译合约）是一种FISCO BCOS底层内嵌的、通过C++实现的高效智能合约，提供包括[分布式权限控制](../../design/security_control/permission_control.md)、[CNS](../../design/cns_contract_name_service.md)、系统属性配置、节点类型配置等功能。PrecompiledService是调用这类功能的API的统称，分为：
...
**功能**：访问FISCO BCOS 2.0+节点[JSON-RPC](../../develop/api.md)；部署合约；调用合约
