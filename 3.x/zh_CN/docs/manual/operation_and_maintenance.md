...
密钥使用时间越长,泄露的可能性就越大。若密钥已泄露,密钥使用越久,损失就会越大。 因此对密钥的使用，设置有效期，当密钥超过有效期或停止使用后,对该密钥进行销毁，具体销毁流程如下：
1. 检查节点通信密钥有效期，若密钥过期，将该密钥进行归档后销毁，如果密钥停止使用，也可将密钥对当，进行主动销毁；
2. 更新节点TLS通信密钥，产生新的通信密钥；
3. 将新密钥进行备份，重启节点，启用新密钥。
