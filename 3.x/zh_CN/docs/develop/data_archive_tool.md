...
数据归档工具位于源码`tools/archive-tool/archiveTool.cpp`，编译时设置参数`cmake -DTOOLS=ON ..`，编译后的二进制位于`build/tools/archive-tool/archiveTool`，需要注意归档工具需要在节点路径下运行，归档工具使用说明如下：
...
`-r`选项表示执行数据归档操作，参数是`[起始区块] [结束区块]`，其中结束区块不会被导入。`-p`选项表示从rocksdb导入，参数是rocksdb路径，如果需要从TiKV导入，则使用`--pd`参数。操作示例如下：
...
< content-length: 865
< content-type: application/json
< date: Wed, 23 Nov 2022 10:04:51 GMT
<
...
$ curl -X POST --data '{"jsonrpc":"2.0","method":"getTransactionReceipt","params":["0x818d1799160d4756bda3d541b4adf2978ffe2d56486c816f82c40d44cce376e8"],"id":1}' http://127.0.0.1:8080 -k -v
Note: Unnecessary use of -X or --request, POST is already inferred.
*   Trying 127.0.0.1:8080...
* Connected to 127.0.0.1 (127.0.0.1) port 8080 (#0)
> POST / HTTP/1.1
...
< content-length: 602
< content-type: application/json
< date: Wed, 23 Nov 2022 10:06:39 GMT
<
* Connection #0 to host 127.0.0.1 left intact
...
$ curl -X POST --data '{"jsonrpc":"2.0","method":"getTransactions","params":[["0x818d1799160d4756bda3d541b4adf2978ffe2d56486c816f82c40d44cce376e8","0xa959b6393419e58d43ca7b61b8fdae77152cd146bd9c1be33c172d4d11a96b89"]],"id":1}' http://127.0.0.1:8080 -k -v
Note: Unnecessary use of -X or --request, POST is already inferred.
*   Trying 127.0.0.1:8080...
* Connected to 127.0.0.1 (127.0.0.1) port 8080 (#0)
> POST / HTTP/1.1
...
< content-length: 1662
< content-type: application/json
< date: Wed, 23 Nov 2022 09:49:45 GMT
<
* Connection #0 to host 127.0.0.1 left intact
...
$ curl -X POST --data '{"jsonrpc":"2.0","method":"getTransactionReceipts","params":[["0x818d1799160d4756bda3d541b4adf2978ffe2d56486c816f82c40d44cce376e8","0xa959b6393419e58d43ca7b61b8fdae77152cd146bd9c1be33c172d4d11a96b89"]],"id":1}' http://127.0.0.1:8080 -k -v
Note: Unnecessary use of -X or --request, POST is already inferred.
*   Trying 127.0.0.1:8080...
* Connected to 127.0.0.1 (127.0.0.1) port 8080 (#0)
> POST / HTTP/1.1
...
< content-length: 1147
< content-type: application/json
< date: Wed, 23 Nov 2022 09:50:42 GMT
<
* Connection #0 to host 127.0.0.1 left intact
