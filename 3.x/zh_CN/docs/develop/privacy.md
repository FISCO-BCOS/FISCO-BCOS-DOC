...
隐私模块的代码和用户开发的预编译合约都位于`FISCO-BCOS/bcos-executor/src/precompiled/extension`目录，因此隐私模块的调用方式和用户开发的预编译合约[调用流程](https://fisco-bcos-documentation.readthedocs.io/zh_CN/latest/docs/manual/smart_contract.html#id12)相同，不过有两点需要注意：
