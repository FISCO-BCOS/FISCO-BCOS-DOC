...
- `-A`: 将开启权限设置，并使用`get_account.sh`与`get_gm_account.sh`脚本随机生成一个账号地址，并将生成账号的公私密钥对放置在链的`ca`目录中，账号的创建和使用请参考：[创建和使用账户](./account.md)
- `-a`：将开启权限设置，并指定一个账号地址作为初始化治理委员的唯一账号，**在指定时，必须确认账号是存在的且账号地址是正确的，否则将会因为没有治理委员权限而导致权限治理不可用的情况**。

#### 开启权限治理举例

使用`-A`选项开启权限模式，可以看到`Auth Mode`已经开启，`Auth init account`初始账号为`0x2690ef01645972e0940400a4bb43f62eb4e2b7f1`。
...
# 随机生成的账号将放置在 {节点名}/ca/accounts 目录下
ls nodes/ca/accounts
0x2690ef01645972e0940400a4bb43f62eb4e2b7f1.pem        0x2690ef01645972e0940400a4bb43f62eb4e2b7f1.public.pem
```

使用`-a`选项开启权限模式，指定账号地址为初始化的治理委员，可以看到`Auth Mode`已经开启，`Auth init account`初始账号为`0x2690ef01645972e0940400a4bb43f62eb4e2b7f1`
...
在这里我们选择开启`auth_check`配置项，并使用`0x2690ef01645972e0940400a4bb43f62eb4e2b7f1`账号地址作为初始化治理委员，请以实际情况为准。
...
控制台有提供权限治理专属的命令，以及切换控制台账号的命令。用户可以通过控制台操作权限的治理，详情请参考：[权限操作命令](./console/console_commands.html#id14)。权限治理的命令只有控制台连入了开启权限治理的节点才会出现。
...
- [openMethodAuth](./console/console_commands.html#openmethodauth)：合约管理员开启某个账号对某个接口的权限；
- [closeMethodAuth](./console/console_commands.html#closemethodauth)：合约管理员关闭某个账号对某个接口的权限；

## 使用举例

首先使用build_chain.sh脚本搭建权限模式的区块链：

这里指定初始化治理委员的账号地址为 0x1cc06388cd8a12dcf7fb8967378c0aea4e6cf642

可以使用`-A`选项自动生成一个账号。账号是有国密和非国密区分的，会根据链的类型自动生成。
...
切换到另外一个委员会账号中，对提案6进行投票，可以看到投票成功，提案状态变更到结束。部署策略也变为白名单模式。
...
可以看到，通过投票，`0x1cc06388cd8a12dcf7fb8967378c0aea4e6cf642` 账号已经有权限部署了。

此时，HelloWorld合约 0x33E56a083e135936C1144960a708c43A661706C0 地址的合约管理员是 `0x1cc06388cd8a12dcf7fb8967378c0aea4e6cf642` 账号

### 3.3 合约管理员使用

接上文，当前HelloWorld合约 0x33E56a083e135936C1144960a708c43A661706C0 地址的合约管理员是 `0xab835e87a86f94af10c81278bb9a82ea13d82d39` 账号
