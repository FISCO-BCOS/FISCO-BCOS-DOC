...
- No calls to other function interfaces
...
- The interface accesses global mapping. The key of mapping is a mutually exclusive parameter
...
- Interface parameters only: string, address, uint256, int256 (more types will be supported in the future)
...
- Web3SDK Client
- One FISCO BCOS chain

If the maximum performance of pressure measurement is required, at least:

- 3 Web3SDKs to generate enough transactions
- 4 nodes, and all Web3SDKs are configured with all node information on the chain, so that transactions are evenly sent to each node, so that the link can receive enough transactions
...
```
# Parameters:<groupID> add <Number of users created> <TPS for this create operation request> <File name of generated user information>
...
```
# Parameters:<groupID> transfer <Total number of transactions> <Maximum TPS for this transfer operation request> <Required User Information File> <Transaction mutual exclusion percentage: 0 ~ 10>
...
```
cd tools
sh get_tps.sh log/log_2019031821.00.log 21:26:24 21:26:59 # Parameters:<Log File> <Calculate Start Time> <Calculation end time>
```
...
```
cd tools
sh get_tps.sh log/log_2019031311.17.log 11:25 11:30 # Parameters:<Log File> <Calculate Start Time> <Calculation end time>
```
