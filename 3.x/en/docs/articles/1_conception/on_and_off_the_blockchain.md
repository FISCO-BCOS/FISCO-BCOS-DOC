...
At present, the underlying blockchain platform generally adopts the "Key-Value" storage structure, which has the advantage of high read and write efficiency, but it is difficult to support complex queries。
...
Some of the logic related to intensive computation should be implemented off-chain as much as possible, such as complex encryption and decryption algorithms, which can be designed to generate off-chain logic for quick verification on the proof chain；If the business process involves the traversal, sorting, and statistics of various data, the index is established under the chain, and only the key-value is accurately read and written on the chain。
