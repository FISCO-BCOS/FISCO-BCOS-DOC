...
The following interface distances are all asynchronous and the interface of the specified node is taken as an example。
...
Query contract code information corresponding to a specified contract address asynchronously。
...
Query contract ABI information corresponding to a specified contract address asynchronously。
...
Obtain the data version number of the current blockchain。
...
```
## 5. Other function interface

### 5.1 set/getExtraData

Additional fields can be written to transactions for use only as storage。

### 5.2 isWasm

Determine if the execution virtual machine on the chain is WASM or EVM

### 5.3 isAuthCheck

Determine whether there is a permission check on the chain

### 5.4 isEnableCommittee

Determine if there are enabled governance members in the chain

### 5.5 isSerialExecute

Determine whether serial execution is on the chain
