...
- `node.*`: All nodes' `IP:Port` or `DomainName:Port` which need to be connected to node. This option supports domain names, but suggests users who need to use it [manually compile source code](https://fisco-bcos-documentation.readthedocs.io/zh_CN/latest/docs/blockchain_dev/get_executable.html#id2).
...
.. important::
    ...
    - Since genesis configuration requires all nodes in the group to be consistent, it is recommended to use `build_chain <../tools/build_chain.html>`_ to generate the configuration.
```
