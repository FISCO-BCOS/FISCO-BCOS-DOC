...
```eval_rst
.. important::
The client requiring to call the contract code which is converted to Java file, needs to put TableTest.sol and Table.sol into the src/test/resources/contract directory of web3sdk, and TableTest.java is generated by the compile script of Web3SDK.
```
...
Create HelloWorldPrecompiled.sol file under console solidity/contracts with the content of interface declaration:
...
Now, we try to create precompiled contract object and call its interface in Solidity contract. Create HelloWorldHelper.sol file in console solidity/contracts with the following content:
